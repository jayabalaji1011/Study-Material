...
function renderPracticeTab(data, lang) {
  ...
  tab.innerHTML = `
    <div class="section-card" style="margin-bottom:20px">
      ...
      <p>Solve these problems to solidify your understanding. Try on your own before revealing the solution!</p>
    </div>
    ${items.map((p, i) => `
      <div class="practice-card">
        <div class="practice-header">
          <div class="example-num">${i+1}</div>
          <span class="difficulty-badge diff-${(p.difficulty||'easy').toLowerCase()}">${p.difficulty || 'Easy'}</span>
        </div>
        <div class="practice-body">
          <div class="practice-question">${p.question}</div>
          ${p.hint ? `<div class="practice-hint">💡 Hint: ${p.hint}</div>` : ''}
          <button class="show-solution-btn" onclick="toggleSolution(this)">Show Solution</button>
          <div class="solution-block">
            ${makeCodeBlock(p.solution, lang)}
          </div>
        </div>
      </div>
    `).join('')}
...
function renderInterviewTab(data) {
  const tab = document.getElementById('interviewTab');
  const qa = data.interview_qa || [];
  tab.innerHTML = `
    <div class="section-card" style="margin-bottom:20px">
      <div class="section-title"><span class="s-icon">🎯</span> Interview Questions & Answers</div>
      <p>Commonly asked interview questions on this topic. Click any question to reveal the answer.</p>
    </div>
    ${qa.map((item, i) => `
      <div class="interview-card" onclick="toggleInterview(this)">
        <div class="interview-question-row">
          <div class="iq-num">${i+1}</div>
          <div class="iq-content">
            <div class="iq-question">${item.question}</div>
            <div class="iq-tags">${(item.tags||[]).map(t=>`<span class="iq-tag">${t}</span>`).join('')}</div>
          </div>
  ...
  return `
    <div class="code-block">
      <div class="code-header">
        <span class="code-lang-badge">${lang === 'sql' ? 'SQL' : 'Python'}</span>
        <button class="copy-btn" onclick="copyCode(this, \`${escapeBacktick(code)}\`)">Copy</button>
      </div>
      <pre>${highlighted}</pre>
    </div>
  `;
...
function syntaxHighlight(code, lang) {
  ...
  if (lang === 'python') {
    // Keywords
    escaped = escaped.replace(/\b(def|class|if|elif|else|for|while|return|import|from|as|try|except|finally|with|pass|break|continue|yield|lambda|not|and|or|in|is|None|True|False|global|nonlocal|raise|del|assert|async|await)\b/g, '<span class="kw">$1</span>');
    // Builtins
    escaped = escaped.replace(/\b(print|len|range|type|int|str|float|list|dict|set|tuple|bool|input|open|enumerate|zip|map|filter|sorted|reversed|sum|min|max|abs|round|isinstance|hasattr|getattr|setattr|super|property|staticmethod|classmethod)\b(?=\s*\()/g, '<span class="bi">$1</span>');
    // Strings
    escaped = escaped.replace(/(&#34;&#34;&#34;[\s\S]*?&#34;&#34;&#34;|&#34;[^&#34;]*&#34;|&#39;[^&#39;]*&#39;)/g, '<span class="str">$1</span>');
    // Comments
    ...
    // SQL
    escaped = escaped.replace(/\b(SELECT|FROM|WHERE|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|DROP|ALTER|ADD|COLUMN|INDEX|VIEW|JOIN|INNER|LEFT|RIGHT|FULL|OUTER|ON|GROUP BY|ORDER BY|HAVING|DISTINCT|AS|UNION|ALL|EXISTS|IN|NOT|AND|OR|IS|NULL|LIKE|BETWEEN|LIMIT|OFFSET|COUNT|SUM|AVG|MIN|MAX|PRIMARY|KEY|FOREIGN|REFERENCES|UNIQUE|DEFAULT|NOT NULL|AUTO_INCREMENT|CONSTRAINT|WITH|CTE|OVER|PARTITION|RANK|ROW_NUMBER|DENSE_RANK|BEGIN|COMMIT|ROLLBACK|TRANSACTION|PROCEDURE|FUNCTION|TRIGGER|RETURNS|RETURN|DECLARE|CURSOR|FETCH|IF|THEN|ELSE|END|CASE|WHEN|TRUNCATE|GRANT|REVOKE)\b/gi, '<span class="kw">$1</span>');
    // Strings
...
function updateTopicProgress() {
  ...
  document.getElementById('topicProgressFill').style.width = pct + '%';
}

function updateMarkDoneBtn(topicId) {
  const btn = document.getElementById('markDoneBtn');
  const isDone = state.completedTopics.has(topicId);
...
document.getElementById('markDoneBtn').addEventListener('click', () => {
  ...
  if (state.completedTopics.has(state.activeTopic)) {
    state.completedTopics.delete(state.activeTopic);
  } else {
    state.completedTopics.add(state.activeTopic);
  }
  localStorage.setItem('pysql_completed', JSON.stringify([...state.completedTopics]));
  updateMarkDoneBtn(state.activeTopic);
  updateTopicProgress();
  renderSidebar(state.activeLang);
  // Re-mark active
  document.querySelectorAll('.nav-item').forEach(el => {
    el.classList.toggle('active', el.dataset.id === state.activeTopic);
  });
});
...
function updateNavArrows() {
  const idx = state.activeTopicIndex;
  document.getElementById('prevTopic').disabled = idx <= 0;
  document.getElementById('nextTopic').disabled = idx >= state.allTopics.length - 1;
}

document.getElementById('prevTopic').addEventListener('click', () => {
  if (state.activeTopicIndex > 0) {
    const prev = state.allTopics[state.activeTopicIndex - 1];
    // Switch lang if needed
    if (prev.lang !== state.activeLang) {
      state.activeLang = prev.lang;
      renderSidebar(prev.lang);
      document.querySelectorAll('.lang-tab').forEach(t => {
        t.classList.toggle('active', t.dataset.lang === prev.lang);
      });
    }
    openTopic(prev.id, prev.lang);
  }
});

document.getElementById('nextTopic').addEventListener('click', () => {
  if (state.activeTopicIndex < state.allTopics.length - 1) {
    const next = state.allTopics[state.activeTopicIndex + 1];
    if (next.lang !== state.activeLang) {
      state.activeLang = next.lang;
      renderSidebar(next.lang);
      document.querySelectorAll('.lang-tab').forEach(t => {
        t.classList.toggle('active', t.dataset.lang === next.lang);
      });
    }
    openTopic(next.id, next.lang);
  }
});
...
document.getElementById('backBtn').addEventListener('click', () => {
  ...
  document.getElementById('topbarTitle').textContent = 'Python & SQL Master';
});
...
document.querySelectorAll('.lang-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    renderSidebar(tab.dataset.lang);
  });
});
...
document.getElementById('modeBtn').addEventListener('click', () => {
  ...
  localStorage.setItem('pysql_theme', isLight ? 'light' : 'dark');
});
...
document.getElementById('searchInput').addEventListener('input', function() {
  ...
  if (!q) {
    ...
    document.querySelectorAll('.nav-category').forEach(el => el.classList.remove('collapsed'));
    return;
  }
  document.querySelectorAll('.nav-item').forEach(el => {
...
// ============================================================
function generateFallbackContent(topic, lang) {
  const isSQL = lang === 'sql';
  return {
    definition: `${topic.title} is a fundamental ${lang.toUpperCase()} concept used in software development and data management.`,
    purpose: `${topic.title} helps developers write efficient, readable, and maintainable code. It is widely used in industry for building applications and managing data.`,
    where_used: [
    ...
    key_concepts: [
      `Core syntax of ${topic.title}`,
      "Common patterns and best practices",
      "Error handling and edge cases",
      "Performance considerations"
    ],
    syntax: isSQL
      ? `-- ${topic.title} basic syntax\nSELECT column1, column2\nFROM table_name\nWHERE condition;`
      : `# ${topic.title} basic syntax\ndef example_function():\n    # Your code here\n    pass`,
    examples: [
      {
        title: "Basic Example",
        description: `A simple demonstration of ${topic.title}`,
        code: isSQL
          ? `-- Basic ${topic.title} example\nSELECT * FROM employees\nWHERE department = 'Engineering';`
          : `# Basic ${topic.title} example\nprint("Hello from ${topic.title}")`,
        output: isSQL ? "Returns matching rows" : `Hello from ${topic.title}`,
        realworld: "Used in real-world applications for data retrieval and processing"
      }
    ],
    practice: [
      {
        difficulty: "Easy",
        question: `Write a basic ${topic.title} example that demonstrates the core concept.`,
        hint: `Start with the simplest possible implementation of ${topic.title}.`,
        solution: isSQL ? `SELECT * FROM table_name;` : `# Solution\nprint("${topic.title} example")`
      }
    ],
    interview_qa: [
      {
        question: `What is ${topic.title} in ${lang.toUpperCase()}?`,
        answer: `${topic.title} is a core ${lang.toUpperCase()} concept. It is used to organize and manage code/data effectively in software applications.`,
        tags: [lang, "basics"]
      },
      {
        question: `When should you use ${topic.title}?`,
        answer: `You should use ${topic.title} when you need to handle specific scenarios in your ${lang.toUpperCase()} code. It is particularly useful in production systems where reliability matters.`,
        tags: ["when-to-use", "best-practices"]
      }
    ]
  };
}
...
function init() {
  buildFlatTopicList();
  renderSidebar('python');
  renderHomeCards();
  updateTopicProgress();

  // Restore theme
  if (localStorage.getItem('pysql_theme') === 'light') {
